...
export default function CheckoutPage() {
  ...
  const loadCheckoutData = useCallback(async () => {
    try {
      // Retrieve selected cart item package IDs from localStorage
      let selectedIds: number[] = [];
      const savedIdsStr = localStorage.getItem('selected_cart_item_ids');
      if (savedIdsStr) {
        try {
      ...
      // Construct cart package IDs suffix for GET requests
      let packageIdsParams = '';
      if (selectedIds.length > 0) {
        selectedIds.forEach((id) => {
          packageIdsParams += `&cart_package_ids[]=${id}`;
        });
      }
      ...
      // Filter locally for maximum safety
      const filteredFlatItems = selectedIds.length > 0 
        ? flatItems.filter((itm) => selectedIds.includes(itm.cart_item_id))
        : flatItems;
      setCheckoutItems(filteredFlatItems);

      // Get grouped delivery details list
      const checkoutRes = await api.get<{ status: boolean; data: any[] }>(`/api/v1/checkout/items${selectedIds.length > 0 ? '?' + packageIdsParams.slice(1) : ''}`)
        .catch(() => ({ status: false, data: [] }));
      
      const rawDeliveryGroups = checkoutRes.data || [];
      const filteredDeliveryGroups = rawDeliveryGroups.map((group: any) => {
        const filteredBody = (group.body || []).filter((pkg: any) => {
          const pkgId = pkg.cartItemId || pkg.cart_item_id || pkg.id;
          return selectedIds.length === 0 || selectedIds.includes(pkgId);
        });
        return {
          ...group,
          body: filteredBody,
        };
      }).filter((group: any) => group.body && group.body.length > 0);
      setDeliveryGroups(filteredDeliveryGroups);
      ...
      if (summaryRes) {
        ...
        setSummary({
          ...
          shippingMethodId: rawSummary.shipping_method_id || rawSummary.shippingMethodId || 0,
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (selectedIds.length > 0) {
        payload.cart_package_ids = selectedIds;
      }
